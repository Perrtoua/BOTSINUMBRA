  ...
  Turbo.on('group-participants-update', async (anu) => {
      ...
      if (anu.action == 'add') {
	    ...
	    capt = welc.replace('@user', tag).replace('@name', username).replace('@bio', about).replace('@date', tanggal).replace('@desc', descrip).replace('@group', mdata.subject);
	      Turbo.send2ButtonLoc(mdata.id, buff, capt, 'Cuenta Oficial De Telegram\nhttps://t.me/Umbraoficial', '⦙☰ MENU', '/menu', '⏍ INFO GP', '/infogp', false, {
	      contextInfo: {  
        ...
        Turbo.sendButtonLoc(mdata.id, buff, capt, 'Cuenta Oficial De Telegram\nhttps://t.me/Umbraoficial', '👋🏻', 'unde', false, {
	      contextInfo: { 
            mentionedJid: Turbo.parseMention(capt)
	      } 
	    });
